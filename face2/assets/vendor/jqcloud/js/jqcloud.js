	...
	$.fn.jQCloud = function(word_array, options) {
		...
		var drawWordCloud = function() {
			...
			var drawOneWord = function(index, word) {
				...
				word_span = $('<span>').attr(word.html).addClass('w' + word_array[index].weight + ' '+ 'c' + word_array[index].type );
				...
				var width = word_span.width(),
					...
					top = options.center.y - height / 1.8;
				...
				while (hitTest(word_span[0], already_placed_words)) {
					...
					if (options.shape === "rectangular") {
						steps_in_direction++;
						if (steps_in_direction * step > (1 + Math.floor(quarter_turns / 1.5)) * step * ((quarter_turns % 4 % 2) === 0 ? 1 : aspect_ratio)) {
							steps_in_direction = 0.0;
							quarter_turns++;
						}
						...
						top = options.center.y + radius * Math.sin(angle) - (height / 1.5);
					}
					word_style.left = left + "px";
					word_style.top = top + "px";
				}
