...
(function (a) {
	...
	Draggable.prototype = {
		...
		_events: function () {
			...
			this._bindEvent(a(document), "move",
				function (c) {
					if (b.is.drag) {
						c.stopPropagation();
						c.preventDefault();
			...
			this._bindEvent(a(document), "down",
				function (c) {
					if (b.is.drag) {
						c.stopPropagation();
						c.preventDefault()
		...
		generateScale: function () {
			if (this.settings.scale && this.settings.scale.length > 0) {
				...
				for (var d = 0; d < e.length; d++) {
					f += '<span style="left: ' + d * g + '%">' + (e[d] != "|" ? "<ins class='jslider-ins'>" + e[d] + "</ins>" : "") + "</span>"
				}
				return f
			} else {
				return ""
