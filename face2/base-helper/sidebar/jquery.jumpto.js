...
! function($) {
    ...
    $.fn.jumpto = function(options) {
        var settings = $.extend({}, defaults, options),
            el = $(this).addClass('jumpto-container'),
            html = "",
        ...
        redrawMenu = function() {
            $(selectors.slice(0, -2)).each(function(index) {
                if (isScrolledIntoView($(this))) {
                    var curLink = $("a[href='#" + $(this).attr("id") + "']");
                    $(".jumpto-subnav a").removeClass("active").parent().find(" a[href='#" + $(this).attr("id") + "']").addClass("active")

                    if (curLink.parent().parent().hasClass("jumpto-second")) {
                        curLink.closest(".jumpto-second").show()
                    }
                    if (curLink.parent().parent().hasClass("jumpto-first")) {
                        curLink.closest(".jumpto-first").find(".jumpto-second").hide()
                    }
                    if (curLink.parent().find(".jumpto-second")) {
                        curLink.parent().find(".jumpto-second").show()
                    }
                    if (curLink.position().top < 0 || curLink.position().top > $(".jumpto-subnav").height()) {
                        $(".jumpto-subnav").scrollTop($("a[href='#" + $(this).attr("id") + "']").position().top);
                    }
                }
            });
        ...
        if (settings.closeButton != false) {
            var close = "<div class='jumpto-close'><a href='javascript:;' id='jumpto-close'>Close</a></div>"
        }
        ...
        $(".jumpto-subnav #jumpto-close").click(function() {
            var $navBody = $(this).closest('.jumpto-subnav').find("> .jumpto-first");

            if (!$navBody.is(":visible")) {
                $(this).html("Close");
                $('.jumpto-container').removeClass('close').animate({
                    'margin-right': $('.jumpto-subnav').width()
                });
            } else {
                $(this).html("Open");
                $('.jumpto-container').addClass('close').animate({
                    'margin-right': 0
                });

            }
            $navBody.slideToggle(400, function() {
                return false;
            });
        });
