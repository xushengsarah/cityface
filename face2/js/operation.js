(function (window, $) {
    ...
    function loadzTreeData($container) {
        var port = 'v2/user/getUserModuleAuthorty',
            ...
            successFunc = function (data) {
                ...
                if (data.code === '200') {
                    loadzTree(data.data);
                    loadPage($('#operationContentPage'), data.data[0].url);
                }
            };
    ...
    function loadzTree(data) {
        var setting = {
            ...
            callback: {
                onClick: function (event, treeId, treeNode) {
                    loadPage($('#operationContentPage'), treeNode.moduleurl);
                    // 二级菜单 选中样式调整
        ...
        $(document).ready(function () {
            $.fn.zTree.init($('#operation-tree-list'), setting, zTreeNodes);
            var treeObj = $.fn.zTree.getZTreeObj("operation-tree-list");
            var nodes = treeObj.getNodes();
            treeObj.selectNode(nodes[0]); //系统管理树状结构默认第一个选中
            $('#operation-tree-list').find('li').first().addClass('active'); // 系统管理树状结构默认第一个选中的样式
        })
    }
    loadzTreeData($("#operationContentPage"));
